...
class PreviewCommand
{
    ...
    public function handle(array $args): void
    {
        ...
        if (!$name) {
            echo "\e[31m❌ Error: Specify a provider (e.g. php foundry preview PostcodeId)\e[0m\n";
            return;
        }

        $className = $this->resolveProviderClass($name);
        if (!$className || !class_exists($className)) {
            echo "\e[31m❌ Error: Provider '{$name}' not found.\e[0m\n";
            return;
        }

        $provider = new $className();

        echo "\e[1;36m🍪 ApiCrumbs Preview: {$name}Provider\e[0m\n";
        ...
        try {
            ...
            if (empty($data)) {
                echo "\e[33m⚠️  Warning: Provider returned empty data.\e[0m\n";
                return;
            }
    ...
    private function resolveProviderClass(string $name): ?string
    {
